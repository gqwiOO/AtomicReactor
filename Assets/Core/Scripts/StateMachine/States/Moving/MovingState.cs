...
namespace Core.Scripts.StateMachine.States.Moving
{
    public class MovingState: MonoBehaviour, IPayloadState<MovingStateData>
    {
        ...
        public async UniTask Enter(MovingStateData duration)
        {
            _animationState.Enter(new AnimationStateData(duration.AnimationKey, AnimationConstants.IdleTrigger)).Forget();
            
            if (duration.LookAt)
                await _moveTarget.DOLookAt(duration.TargetPoint, 0.5f);
            
            await _moveTarget.DOMove(duration.TargetPoint, duration.Speed).SetSpeedBased().SetEase(Ease.Linear);
